...
Match.getMatchesReview = (results) => {
  ...
  JSON_EXTRACT(info, "$.matchesAssigned") < JSON_EXTRACT(info, "$.capacity") `, (err, judges) => {
    ...
    sql.query(`SELECT m.nomineeID, GROUP_CONCAT(m.judgeID) AS judgeIDs
    FROM Matches AS m
    INNER JOIN Nominees AS n ON m.nomineeID = n.ID
    WHERE n.Cohort = (SELECT MAX(id) FROM Cohort) AND m.matchStatus = 'm300'
    GROUP BY m.nomineeID;`, (err, currentMatches) => {
    if (err) {
      console.log("error: ", err);
      results(null, err);
      return;
